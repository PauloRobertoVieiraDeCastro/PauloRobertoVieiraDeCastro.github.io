...
ent_rea_bat.addEventListener("click", function(event) {
	...
	conc_rea_bat.placeholder = "C(mol/m³)";
...
ent_prod_bat.addEventListener("click", function(event) {
	...
	conc_prod_bat.placeholder = "C(mol/m³)";
...
function calculoBatelada(){
    var X = 0.7;//parseFloat(document.getElementById("conversao").value*0.01);
    var k0 = 1.0455E11;//parseFloat(document.getElementById("k_rea").value);
    var Tc = 303;
    var Te = 313;
    var T = 320;
    var Hr = 13000;
    ...
    if (isNaN(X) || isNaN(k0)){
       return alert("Nem todos os campos foram preenchidos.");
        ...
        V = Q*(creagB[j] - CreagfB[j])/vv_rB;
        A = (vv_rB*V*Hr + Q*rho*Cp*(Te - T))/(U*(T - Tc));
        document.querySelector("#volume").innerHTML = V.toFixed(1) + " m³"; 
        document.querySelector("#area").innerHTML = A.toFixed(1) + " m²"; 
        console.log('volume '+V)
        console.log(creagB[j]);
        console.log(CreagfB[j])
        console.log('kh'+vv_rB*V*Hr)
        console.log('q'+Q*rho*Cp*(Te - T))
        console.log('area '+A)
        /*--------------------------PERFIL DA CONVERSÃO TEMPORAL----------------------------------------------------*/
        CONV = [];
        contador = 1;
        tempo = [];
        function temperatura(T,X,Q,rho,Cp,Te,U,A,Tc,k0,creagB,V,Hr,Er,R){
            y = Q*rho*Cp*(T - Te) + U*A*(T - Tc) - k0*creagB*(1 - X)*V*Hr*Math.exp(-Er/(R*T));
            return y
        }
        function df(temperatura,T,X,Q,rho,Cp,Te,U,A,Tc,k0,creagB,V,Hr,Er,R){
            dff = (temperatura(T+0.0001,X,Q,rho,Cp,Te,U,A,Tc,k0,creagB,V,Hr,Er,R)-temperatura(T-0.0001,X,Q,rho,Cp,Te,U,A,Tc,k0,creagB,V,Hr,Er,R))/0.0002;
            return dff;
        }

        Temper = []
       for(var iJ=0;iJ<=2000;iJ++){
            CA = 0.1*iJ*creagB[j];
            CONV.push(CA);
            for(var i = 0; i<=200;i++){
                T0 = 250;
                T = T0 - temperatura(T0,X,Q,rho,Cp,Te,U,A,Tc,k0,CA,V,Hr,Er,R)/df(temperatura,T0,X,Q,rho,Cp,Te,U,A,Tc,k0,CA,V,Hr,Er,R);
                erro = Math.abs((T - T0)/T);
                if(erro<=0.001){
                    break;
                }
                T0 = T;
