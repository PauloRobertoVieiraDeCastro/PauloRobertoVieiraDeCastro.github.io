//import {densidade} from "./densidade.js"
...
function balanco_mvd_simples(){
	var rho_oleo_exp = parseFloat(document.querySelector("#rho_oleo").value);
	var m_oleo = parseFloat(document.querySelector("#m_oleo").value);
	var percmas_lev = parseFloat(document.querySelector("#perc_mas").value)*0.01;
	var percvol_lev = parseFloat(document.querySelector("#perc_vol").value)*0.01;
	...
	var rho_res_exp = parseFloat(document.querySelector("#rho_res_exp").value);
	var T_res_exp = parseFloat(document.querySelector("#temp_res").value);
	var T_o_exp = parseFloat(document.querySelector("#temp_oleo").value);
	//rho_oleo_exp = 0.8785;
	/*T_o_exp = 20;
	m_oleo = 3000;
	...
	if (isNaN(rho_oleo) || isNaN(m_oleo) || isNaN(mas_dest) || isNaN(mas_res) || isNaN(rho_res_exp) || isNaN(vol_dest)){
	...
	delta_res = densidade(rho_res_exp,T_o_exp)[1]*(percm_perda + percm_res) + densidade(rho_dest,20)[1]*(percm_dest) + 100*(percmas_lev);
	delta_dest = densidade(rho_res_exp,T_o_exp)[1]*(percm_res) + densidade(rho_dest,20)[1]*(percm_perda + percm_dest) + 100*(percmas_lev);
	if(mas_lev === 0.0 || vol_lev === 0.0){
		delta = {'Destilado':Math.abs(delta_dest-api_amostra),'Resíduo':Math.abs(delta_res-api_amostra)}
	}
	else{
		delta_lev = densidade(rho_res_exp,T_o_exp)[1]*(percm_res) + densidade(rho_dest,20)[1]*(percm_dest) + 100*(percm_perda + percmas_lev);
