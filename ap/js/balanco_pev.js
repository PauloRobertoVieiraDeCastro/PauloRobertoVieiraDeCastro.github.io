...
function calcularPotstill(){
	...
	this.T_topo_totalp = [];
	this.soma_vol_totalp = [];
	rho_oleo_exp = parseFloat(document.querySelector("#rho_oleo").value);//0.8785 - densidade do residuo mvd
	T_o_exp = parseFloat(document.querySelector("#temp_oleo").value);//20 temp residuo mvd
...
function PEV(){
	...
	var li = []
    for (let i = 0; i < curva_topo.length; i++) {
        var obj = {};
        obj['x'] = 100*curva_vol[i];
        obj['y'] = curva_topo[i];
        //obj['y2'] = curva_api[i];
        li.push(obj)
    }
