...
function calcular(){
	...
	if(metodologia == 0){
		raiz = Newton_Raphson(f,x0,tol,iter);	
	}
	if(metodologia == 1){
		var x1 = parseFloat(document.querySelector("#x1").value);
		raiz = Secante(f,x0,x1,tol,iter);
	}
	if(metodologia == 2){
		var x1 = parseFloat(document.querySelector("#x1").value);
		raiz = Bisseccao(f,x0,x1,tol,iter);
	}
	if(metodologia == 3){
		var x1 = parseFloat(document.querySelector("#x1").value);
		raiz = RegulaFalsi(f,x0,x1,tol,iter);
	}
	if(metodologia == 4){
		raiz = Schroder(f,x0,tol,iter);
	}
	if(metodologia == 5){
		raiz = Halley(f,x0,tol,iter);
	}
	if(metodologia == 6){
		raiz = TSM(f,x0,tol,iter);
	}
	if(metodologia == 7){
		raiz = FLM(f,x0,tol,iter);
	}

	document.querySelector("#resultado").innerHTML = "A raiz da função determinada é: " + raiz.toFixed(3);
	/*--------------------CRIANDO TABELA---------------------------------------------------------------------------------------*/
    ...
    var li = [];
    var li2 = [];
    var xf1 = parseFloat(raiz) + 5.0;//6
    var xf2 = parseFloat(raiz) - 5.0;//-4
