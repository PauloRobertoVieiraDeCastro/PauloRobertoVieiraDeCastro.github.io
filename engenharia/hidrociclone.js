...
function calcularH(){
    ...
    if (isNaN(niH) || isNaN(cargah) || isNaN(CV_CH) || isNaN(rhof_CH) || isNaN(rho_CH) || isNaN(Q_CH) || isNaN(RL)){
        ...
        return alert("Não é possível existir razão de líquido maior do que 1.");
    }
    else if(RL<0){
        return alert("Não é possível existir razão de líquido menor do que 0.");
    }
    else if(niH>1){
        return alert("Não é possível existir eficiência maior do que 100%.");
    }
    else if(niH<0){
        return alert("Não é possível existir eficiência menor do que 0%.");
    }
    else if(this.contagemh<3){
        return alert("Poucos dados disponíveis de destribuição de partículas disponíveis.");
    }
    else{
        if(radios==0){
            d = [];
            v = [];
            for(var i=1001;i<=this.contagemh;i++){
